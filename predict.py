...
def decode_ent(text, pred_matrix, tokenizer,
    ...
    for ent_type_id, token_start_index, token_end_index in zip(*np.where(pred_matrix > threshold)):
        ...
        start = token2char_span_mapping[token_start_index][0]
        end = token2char_span_mapping[token_end_index][1]-1
        ent_char_span = [start, end]
        ent_text = text[ent_char_span[0]:ent_char_span[1]+1]
...
def predict(dataloader, model):
    ...
    for batch_data in dataloader:
        ...
        for ind in tqdm(range(len(batch_samples))):
...
def load_model():
    ...
    model_state_path = os.path.join(model_state_dir, model_state_list[len(model_state_list) - 1])
