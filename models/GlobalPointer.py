...
class MetricsCalculator(object):
    ...
    def get_report(self, y_pred, y_true):
        ...
        return classification_report(pred, true)
...
class GlobalPointer(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, token_type_ids):
        ...
        batch_size = last_hidden_state.size()[0]  # 16
        seq_len = last_hidden_state.size()[1] # 512

        # outputs:(batch_size, seq_len, ent_type_size*inner_dim*2)
        outputs = self.dense(last_hidden_state)
        outputs = torch.split(outputs, self.inner_dim * 2, dim=-1)  # dim=-1表示切分最后一个维度
        # outputs:(batch_size, seq_len, ent_type_size, inner_dim*2)
