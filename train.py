...
def data_generator(data_type="train"):
    ...
    if config["exp_name"] == "jd" or config["exp_name"] == "tp" or config["exp_name"] == "pro":
        if data_type == "train":
            train_data_path = open(os.path.join(config["data_home"], config["exp_name"], config["train_data"]), 'r',
                                   encoding='utf-8')
            train_data = json.load(train_data_path)
            valid_data_path = open(os.path.join(config["data_home"], config["exp_name"], config["valid_data"]), 'r',
                                   encoding='utf-8')
            valid_data = json.load(valid_data_path)
        elif data_type == "valid":
            valid_data_path = open(os.path.join(config["data_home"], config["exp_name"], config["valid_data"]), 'r',
                                   encoding='utf-8')
            valid_data = json.load(valid_data_path)
...
def valid_step(batch_valid, model):
    ...
    # report = metrics.get_report(logits, batch_labels)

    return sample_f1, sample_precision, sample_recall
    # return sample_f1, sample_precision, sample_recall, report
